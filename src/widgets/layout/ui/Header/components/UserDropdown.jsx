...
export const UserDropdown = () => {
  ...
  return (
    <div className="rigth-10 tablet:right-0 absolute top-[34px] z-10 mt-2 flex w-35 flex-col gap-1 rounded-xl border-1 border-[var(--color-white)] bg-[var(--color-dark)] p-4">
      <button className="w-full text-left text-sm text-xs font-bold font-medium text-white uppercase">
        Profile
      </button>
      <button
        className="flex w-full gap-[2px] text-left text-sm text-xs font-bold font-medium text-white uppercase disabled:opacity-60"
        onClick={logout}
        disabled={isPending}
      >
        {isPending ? (
