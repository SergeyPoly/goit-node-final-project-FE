...
export const AddRecipePage = () => {
  ...
  const handleSubmit = useCallback(
    async (values) => {
      if (!(values.recipeImage instanceof File)) {
        toast.error('Recipe photo is required');
        return;
      }
      ...
      try {
        ...
        const message = err?.response?.data?.message ?? err?.message ?? 'Failed to create recipe';
        toast.error(message);
      }
    },
    [navigate]
...
const AddRecipeFormContent = ({
  ...
  const handleAddIngredient = useCallback(() => {
    if (!values.selectedIngredient?.trim() || !values.measure?.trim()) return;
  ...
  }, [values, ingredients, setFieldValue, setFieldTouched, validateForm]);
  ...
  return (
    <div className="desktop:flex-row desktop:gap-20 tablet:gap-20 flex flex-col gap-10">
      <div
        className={cn(
      ...
      <div className="desktop:min-w-0 desktop:max-w-[649px] desktop:flex-1 flex flex-col gap-20">
        <Form className="tablet:gap-10 flex flex-col gap-8">
          {/* Name + Description */}
          ...
          <section className="tablet:gap-14 flex flex-col gap-5" aria-labelledby="category-heading">
            <div className="tablet:flex-row tablet:gap-5 flex flex-col gap-5">
              <div className="flex min-w-0 flex-1 flex-col gap-2">
                <label id="category-heading" className="text-main text-lg font-extrabold uppercase">
                  Category
                </label>
                <Select
                  name="category"
              ...
              <div className="tablet:max-w-[194px] flex min-w-0 flex-shrink-0 flex-col gap-2">
                <label htmlFor="cook-time" className="text-main text-lg font-extrabold uppercase">
                  Cooking time
                </label>
                <div
                  id="cook-time"
            ...
            <div className="tablet:gap-10 flex flex-col gap-10">
              ...
              {values.ingredientsList.length > 0 && (
                <div className="flex flex-wrap gap-4" role="list" aria-label="Added ingredients">
          ...
          >
            <h2 id="preparation-heading" className="text-main text-lg font-extrabold uppercase">
              Recipe Preparation
            </h2>
            <TextField
